...
const Statement = () => {
    ...
    // Group items by voucher for itemwise view
    // const getGroupedItemwiseStatement = () => {
    //     const grouped = {};
    //     itemwiseStatement.forEach(item => {
    //         const voucherKey = `${item.date}-${item.billNumber}`;
    ...
    //                 partyBillNumber: item.partyBillNumber,
    //                 items: []
    //             };
    //         }
    //         grouped[voucherKey].items.push(item);
    //     });
    //     return grouped;
    // };
...
`;
    ...
    //                         const voucherKey = `${item.date}-${item.billNumber}`;
    //                         const voucherItems = groupedStatement[voucherKey]?.items || [];
    //                         const hasItems = voucherItems.length > 0;
    ...
    //                                                     Contains {voucherItems.length} item(s)
    //                                                 </div>
    //                                             )}
    //                                         </div>
    //                                     </td>
    ...
    //                                 {/* Item Details Rows - Always visible in itemwise view */}
    //                                 {/* {hasItems && voucherItems.map((itemDetail, itemIndex) => {
    //                                     // Determine which price to display based on transaction type
    //                                     const isPurchase = item.type?.toLowerCase() === 'purc';
    //                                     const isPurchaseReturn = item.type?.toLowerCase() === 'prrt';
    //                                     const displayPrice = isPurchase || isPurchaseReturn ? itemDetail.puPrice : itemDetail.price;
    //                                     const priceLabel = isPurchase || isPurchaseReturn ? 'Pur. Rate' : 'Rate';
    ...
    //                                                                 <strong>Amount:</strong> {formatCurrency(calculatedAmount)}
    //                                                             </span>
    //                                                         )}
    //                                                     </div>
    //                                                 </div>
    ...
    //                                     // CALCULATE AMOUNTS BASED ON FETCHED DATA
    //                                     const calculatedAmount = (itemDetail.netPrice || 0) * (itemDetail.quantity || 0);
    //                                     const discountAmount = itemDetail.discountAmountPerItem || 0;
    //                                     const vatAmount = itemDetail.vatAmount || 0;
    //                                     const totalAmount = itemDetail.totalAmount || 0;
    ...
    //                                                                 <strong>VAT:</strong> {formatCurrency(vatAmount)}
    //                                                             </span>
    //                                                         )}
    //                                                         {/* TOTAL AMOUNT */}
    //                                                         {totalAmount > 0 && (
    ...
    //                         // Get VAT and Total directly from the first item (same for all items in the bill)
    //                         let totalVatAmount = 0;
    //                         let finalTotalAmount = 0;

    //                         if (hasItems && voucherItems.length > 0) {
    //                             // Use the values directly from the API response
    //                             totalVatAmount = voucherItems[0].vatAmount || 0;
    //                             finalTotalAmount = voucherItems[0].totalAmount || 0;
    //                         }
    ...
    //                                                     {/* Separator Line */}
    //                                                     <div className="border-top pt-1"></div>
...
//         const savedWidths = localStorage.getItem('statementColumnWidths');
//         if (savedWidths) {
//             try {
...
//             }
//         }
//         // Default widths for regular statement view
//         return {
//             date: 90,
//             voucherNo: 100,
//             voucherType: 90,
...
//     useEffect(() => {
//         localStorage.setItem('statementColumnWidths', JSON.stringify(columnWidths));
//     }, [columnWidths]);
...
//                     style={{ width: `${columnWidths.voucherNo}px`, flexShrink: 0, minWidth: '80px' }}
//                 >
...
//                 <div
//                     className="d-flex align-items-center px-2 border-end border-white position-relative"
//                     style={{ width: `${columnWidths.voucherType}px`, flexShrink: 0, minWidth: '70px' }}
//                 >
//                     <strong>Vch Type</strong>
//                     <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.voucherType - 2} columnName="voucherType" />
//                 </div>
...
//                 <div
//                     className="d-flex align-items-center px-2 border-end border-white position-relative"
//                     style={{ width: `${columnWidths.account}px`, flexShrink: 0, minWidth: '120px' }}
//                 >
//                     <strong>Account</strong>
//                     <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.account - 2} columnName="account" />
//                 </div>

//                 {/* Debit */}
//                 <div
//                     className="d-flex align-items-center px-2 border-end border-white position-relative"
//                     style={{ width: `${columnWidths.debit}px`, flexShrink: 0, minWidth: '80px' }}
//                 >
//                     <strong>Debit</strong>
...
//                 <div
//                     className="d-flex align-items-center px-2 border-end border-white position-relative"
//                     style={{ width: `${columnWidths.credit}px`, flexShrink: 0, minWidth: '80px' }}
//                 >
...
//                     style={{ width: `${columnWidths.balance}px`, flexShrink: 0, minWidth: '100px' }}
//                 >
...
//     // Table Row Component for regular view
//     const RegularTableRow = React.memo(({ index, style, data: { statement, selectedRowIndex, formatCurrency, formatAmountWithType, onRowClick, onRowDoubleClick } }) => {
...
//                 <div
//                     className="px-2 border-end d-flex align-items-center"
//                     style={{ width: `${columnWidths.voucherType}px`, flexShrink: 0, height: '100%' }}
//                 >
//                     <span style={{ fontSize: '0.85rem', whiteSpace: 'nowrap' }}>
//                         {item.type}
//                     </span>
...
//                 >
//                     <Badge 
//                         bg={item.paymentMode === 'cash' ? 'success' : 'warning'} 
//                         pill 
...
//                 <div
//                     className="px-2 border-end d-flex align-items-center"
//                     style={{ width: `${columnWidths.account}px`, flexShrink: 0, height: '100%' }}
//                     title={item.accountType?.name || item.purchaseSalesType || item.journalAccountType || item.purchaseSalesReturnType || item.drCrNoteAccountType || item.paymentReceiptAccountType || 'Opening'}
//                 >
//                     <div style={{ width: '100%' }}>
//                         <span style={{
//                             fontSize: '0.85rem',
...
//                                 display: 'block'
//                             }}>
//                                 Inst: {item.InstType} {item.InstNo && `- ${item.InstNo}`}
//                             </span>
//                         )}
//                     </div>
//                 </div>
...
//                     style={{ width: `${columnWidths.debit}px`, flexShrink: 0, height: '100%' }}
//                 >
//                     <span style={{ fontSize: '0.85rem', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>
//                         {formatCurrency(item.debit) || '0.00'}
//                     </span>
//                 </div>
...
//                 <div
//                     className="px-2 border-end d-flex align-items-center justify-content-end"
//                     style={{ width: `${columnWidths.credit}px`, flexShrink: 0, height: '100%' }}
//                 >
//                     <span style={{ fontSize: '0.85rem', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>
//                         {formatCurrency(item.credit) || '0.00'}
//                     </span>
//                 </div>

//                 {/* Balance */}
//                 <div
//                     className="px-2 d-flex align-items-center justify-content-end"
//                     style={{ width: `${columnWidths.balance}px`, flexShrink: 0, height: '100%' }}
//                 >
//                     <span style={{ fontSize: '0.85rem', fontFamily: 'monospace', whiteSpace: 'nowrap', fontWeight: 'bold' }}>
//                         {formatAmountWithType(item.balance) || '0.00'}
//                     </span>
//                 </div>
//             </div>
//         );
...
//         totalWidth += columnWidths.voucherType;
//         totalWidth += columnWidths.paymentMode;
//         totalWidth += columnWidths.account;
//         totalWidth += columnWidths.debit;
//         totalWidth += columnWidths.credit;
//         totalWidth += columnWidths.balance;
//         return totalWidth;
//     };
...
//                     break;
//                 case 'Enter':
//                     if (selectedRowIndex >= 0 && filteredStatement[selectedRowIndex]) {
//                         handleRowDoubleClick(filteredStatement[selectedRowIndex]);
//                     }
//                     break;
//                 default:
//                     break;
...
//                     ) : (
//                         <>
//                             {/* Statement Table - Virtualized for regular view */}
//                             {viewMode === 'regular' ? (
//                                 <>
...
//                                         >
//                                             <i className="fas fa-redo me-1"></i> Reset Columns
//                                         </Button>
//                                     </div>
...
//                                             <div className="text-center p-4">No statement found</div>
//                                         ) : (
//                                             <AutoSizer>
//                                                 {({ height, width }) => {
//                                                     const totalWidth = getTotalTableWidth();
